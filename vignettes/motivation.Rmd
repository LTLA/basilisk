...
This allows users to force `r Biocpkg("basilisk")` to use their own Python installation instead of installing one via `r Biocpkg("reticulate")`.
The same approach can be used with `BASILISK_CUSTOM_PYTHON_X_Y` and `BASILISK_CUSTOM_PYTHON_X` for Python versions `X.Y` or `X`, which will be used to satisfy a request for `X.Y.Z` if the more specific variable is not set.
Note that different client packages may need different Python versions so multiple environment variables may need to be set for different `X`/`Y`/`Z` combinations.
