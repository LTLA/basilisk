...
If `BASILISK_EXTERNAL_CONDA` is set to a read-only conda installation or if both `BASILISK_EXTERNAL_CONDA` and `BASILISK_USE_SYSTEM_DIR` are set,
it may be necessary to set the `BASILISK_EXTERNAL_FALLBACK_R` environment variable. 
This should refer to an absolute path to an existing conda environment containing the fallback R environment.
