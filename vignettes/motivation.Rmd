...
Setting the `BASILISK_CUSTOM_PYTHON_X_Y` environment variable will cause all requests for Python version `X.Y` to use the Python binary at the specified path.
...
Note that different client packages may need different Python versions so multiple environment variables may need to be set for different `X` and `Y`. 
