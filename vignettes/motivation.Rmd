...
If `BASILISK_EXTERNAL_CONDA` is set to a read-only conda installation or if both `BASILISK_EXTERNAL_CONDA` and `BASILISK_USE_SYSTEM_DIR` are set, the fallback R environment will not be automatically created.
In such cases, the `BASILISK_EXTERNAL_FALLBACK_R` environment variable can be set to the absolute path to an existing conda environment containing the fallback environment.
This ensures correct operation for any `r Biocpkg("basilisk")` clients requiring the fallback. 
